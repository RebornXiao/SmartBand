...
public class SportTimeActivity extends Activity {
	...
	private TextView tvsportPercent,tvsportTime;
	...
	private void init(){
		...
		tvsportPercent = (TextView)findViewById(R.id.tvsportPercent);
		tvsportTime = (TextView)findViewById(R.id.tvsportTime);
		ArrayList<Sport> sports = model.sportSearth();
		if(sports.size() > 0){
			int index = sports.size() - 1;
			tvsportPercent.setText(sports.get(index).getStepRate());
			tvsportTime.setText(sports.get(index).getSportTime());
		}
		titleView.setBack(R.drawable.steps_back, new TitleView.onBackLister() {
			...
			//	MyApplication.eBbraceletcom.Stop();
				//if(mReceiver != null) SportTimeActivity.this.unregisterReceiver(mReceiver);//取消蓝牙广播
				//   mReceiver = null;
				//   device = null;

				if(mReceiver != null) SportTimeActivity.this.unregisterReceiver(mReceiver);//取消蓝牙广播  && status == 1 
				   mReceiver = null; 
				finish();
			}
		});
		...
		MyApplication.eBbraceletcom.Start(new EBEventHander(){
						...
						if(nCmdPara[0] == 0x02){
							...
							for(int i = 0 ; i<(nLen -4 - 2)/11; i++){
								...
								if(ndate.equals(MyApplication.ways.getDate())){
									tvsportTime.setText(stepTime[0]*60+stepTime[1]+"");
									tvsportPercent.setText(pedometerPer + "");
			...
			public void OnBtEvent(int nEvent) {
				...
				if(nEvent == EBbraceletcom.BT_EVENT_DEVICE_DISCONNET){
					...
					Log.i("info", "My.connet="+MyApplication.CONNETET);
					MyApplication.eBbraceletcom.Stop();
					//MyApplication.eBbraceletcom.DisConnect
					try {
						Thread.sleep(500);
				...
				MyApplication.eBbraceletcom.Start(new EBEventHander(){
					public void OnBtEvent(int nEventType){
						...
						if(nEventType == EBbraceletcom.BT_EVENT_DEVICE_SCANNING){
					        ...
					        SportTimeActivity.this.registerReceiver(mReceiver, filter);
