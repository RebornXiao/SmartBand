...
public class PedometerActivity extends Activity {
	...
	private TextView tvsteps,tvpedometerPer;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.bracelet_pedometer);
		init();
	...
	private void init(){
		...
		tvpedometerPer = (TextView)findViewById(R.id.tvpedometerPer);
		...
		if(sports.size() > 0){
		   ...
		   tvsteps.setText(sports.get(index).getStep());
		   tvpedometerPer.setText(sports.get(index).getStepRate());
		}
		titleView.setTitle(R.string.pedometer);
		...
		titleView.setBack(R.drawable.steps_back, new TitleView.onBackLister() {
			...
			public void onClick(View button) {
				...
				if(mReceiver != null) PedometerActivity.this.unregisterReceiver(mReceiver);//取消蓝牙广播  && status == 1
				   mReceiver = null;
			//	   status = 0 ;
				finish();
			}
		...
		titleView.right(R.string.history, new TitleView.onSetLister() {
			...
			public void onClick(View button) {
				...
				MyApplication.activityStuts = 1;
				startActivity(intent);
			}
		});
	}
	
	private void sendCmdSteps(){
...
//				if(lenght <= 17) return;
				//if(cmd != null){// && lenght > 0
					...
					}//else MyApplication.ways.ToastShow(getApplicationContext(), "检验码不对");
				}
			//}
	    }
			@Override
				...
				MyApplication.eBbraceletcom.Start(new EBEventHander(){
					public void OnBtEvent(int nEventType){
						...
						if(nEventType == EBbraceletcom.BT_EVENT_DEVICE_SCANNING){
					        ...
					        PedometerActivity.this.registerReceiver(mReceiver, filter);
	     ...
	     //           MyApplication.address = dev.getAddress();
	           ...
	           if (BluetoothDevice.ACTION_BOND_STATE_CHANGED.equals(action)) {
	            ...
	            if (BluetoothAdapter.ACTION_DISCOVERY_FINISHED.equals(action) && device == null) {
	            	...
	            	if(mReceiver != null) PedometerActivity.this.unregisterReceiver(mReceiver);//取消蓝牙广播
	            	mReceiver = null;
	           }
	        }
	    }
